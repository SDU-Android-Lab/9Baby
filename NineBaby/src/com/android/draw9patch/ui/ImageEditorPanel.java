...
class ImageEditorPanel extends JPanel {
    ...
    private class ImageViewer extends JComponent {
        ...
        private int y1;
        private int y2;
        private void clearAltPoint(){
        	x1=x2;
        	x2=0;
        	y1=y2;
        	y2=0;
        }
        private void drawLine(int x1,int x2,int y1,int y2,int button){
        	if(Math.abs(x1-x2)>Math.abs(y1-y2)){
        		int minX=Math.min(x1, x2);
        		int maxX=Math.max(x1, x2);
        		while(minX<=maxX){
        			paint(++minX, y1, button);
        		}
        	}else{
        		int minY=Math.min(y1, y2);
        		int maxY=Math.max(y1, y2);
        		while(minY<=maxY){
        			paint(x1,++minY, button);
        		}
        	}
        	clearAltPoint();
        }
        ImageViewer() {
            ...
            helpLabel = new JLabel("Press Shift to erase pixels,Press Alt to draw Line");
                ...
                public void mousePressed(MouseEvent event) {
                    ...
                    paint(event.getX(), event.getY(), currentButton);
                    	...
                    	if(!event.isAltDown()){
                    		...
                    		x2=event.getX();
                    		y2=event.getY();
                    		drawLine(x1, x2, y1, y2,currentButton);
        ...
        private void enableEraseMode(KeyEvent event) {
            ...
            if (eraseMode != oldEraseMode) {
                if (eraseMode) {
                    ...
                    helpLabel.setText("Press Shift to erase pixels,and ues alt to erase a line");
                }
            }
        }
